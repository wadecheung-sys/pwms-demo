    ...
    <div class="page-panel">
      <div class="panel-actions">
        <div>
          ...
          <p class="page-desc">覆盖库存/定额、校验、环境、合规四类风险；支持穿透到台账、仓室、定额结果。</p>
        </div>
      ...
      <el-table :data="rows" border stripe>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <el-button link type="primary" @click="go(row)">穿透</el-button>
            <template v-if="canHandle && row.status === '未处理'">
              <el-button link type="success" @click="handle(row, '已处理')">处理</el-button>
              <el-button link @click="handle(row, '已忽略')">忽略</el-button>
            </template>
          </template>
        </el-table-column>
      </el-table>
