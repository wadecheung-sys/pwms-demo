...
import { getExportColumns, getExportFilename } from '@/views/Pwms/asset/assetExportColumns'
import {
  applyAssetFilter,
  buildFilterFields,
  getFilterDefaults,
} from '@/views/Pwms/asset/assetFilters'
import { todayStr } from '@/utils/pwms/id'
import { isInventoryCenterOrg, getOrgDescendantIds } from '@/utils/pwms/org'

async function withLoading<T>(fn: () => T | Promise<T>): Promise<T> {
  try {
    return await fn()
  } finally {
    /* no-op shell loading; keep call sites */
  }
}
...
<template>
  <PageShell tip="按专业/模块维护台账、出入库、故障、维修与盘点；支持导出打印与穿透下钻。">
    ...
    <el-table v-if="subModule === 'ledger'" :data="pageData" stripe border>
        ...
        <el-table-column prop="status" label="状态" width="80">
          <template #default="{ row }">
            <el-tag
              :type="row.status === '在库' ? 'success' : row.status === '在用' ? 'primary' : row.status === '维修中' ? 'warning' : 'info'"
              size="small"
            >{{ row.status }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <div class="table-actions">
              <el-button type="primary" size="small" plain @click="openAssetDetail(row.assetCode)">详情</el-button>
              <template v-if="canEditCurrent">
                <el-button type="primary" size="small" plain @click="openDialog(row)">编辑</el-button>
                <el-button type="danger" size="small" plain @click="handleDelete('台账', () => dataStore.removeLedger(row.id))">删除</el-button>
              </template>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="280" fixed="right">
          <template #default="{ row }">
            <div class="table-actions">
              ...
              <template v-if="canEditCurrent">
                <el-button type="primary" size="small" plain @click="openEditFault(row)">编辑</el-button>
                <el-button type="danger" size="small" plain @click="handleDelete('记录', () => dataStore.removeFaultRecord(row.id))">删除</el-button>
              </template>
            </div>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="220" fixed="right">
          <template #default="{ row }">
            <div class="table-actions">
              <el-button type="primary" size="small" plain @click="openAssetDetail(row.assetCode)">详情</el-button>
              <template v-if="canEditCurrent">
                <el-button type="primary" size="small" plain @click="openEditMaintenance(row)">编辑</el-button>
                <el-button type="danger" size="small" plain @click="handleDelete('记录', () => dataStore.removeMaintenanceRecord(row.id))">删除</el-button>
              </template>
            </div>
          </template>
        </el-table-column>
      ...
      <el-table v-else :data="pageData" stripe border>
        ...
        <el-table-column label="层级" width="100">
          <template #default="{ row }">
            <el-tag :type="row.level === 'center' ? 'primary' : 'success'" size="small">{{ levelLabel(row.level) }}</el-tag>
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="200" fixed="right">
          <template #default="{ row }">
            <div class="table-actions">
              <el-button type="primary" size="small" plain @click="openDrilldown(row)">下钻明细</el-button>
              <el-button
                v-if="canEditCurrent"
                type="danger"
                size="small"
